...
try:
    from scraping.newssites.nrc_nieuws import WebNieuwsNRCScraper
except ImportError:
    from amcatscraping.newssites.nrc_nieuws import WebNieuwsNRCScraper

from amcat.scraping.scraper import ArchiveForm

from datetime import timedelta

class NRCWebArchiveScraper(WebNieuwsNRCScraper):
    options_form = ArchiveForm
    
    def get_units(self):
        self.options['date'] = self.options['first_date']
        while self.options['date'] <= self.options['last_date']:
            for unit in super(NRCWebArchiveScraper,self).get_units():
                yield unit
...
if __name__ == '__main__':
    ...
    cli.run_cli(NRCWebArchiveScraper)
