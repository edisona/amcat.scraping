...
class GoogleSearch(object):
    ...
    SEARCH_URL_1 = "http://www.google.%(tld)s/search?num=100&hl=%(lang)s&q=%(query)s&btnG=Google+Search"
    NEXT_PAGE_1 = "http://www.google.%(tld)s/search?num=100&hl=%(lang)s&q=%(query)s&start=%(start)d"
    ...
    def _get_results_page(self):
        ...
        self.cookies = page.info()['set-cookie'].replace("NR=10","NR=100")
            
        self.browser.opener.addheaders(("Cookie",self.cookies))
